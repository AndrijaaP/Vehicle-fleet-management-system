...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['id']) && isset($_POST['tip'])) {
        ...
        if ($query->execute()) {
            header("Location: edit.php?id=$id&tip=$tip&success=" . urlencode("Izmene su uspešno sačuvane!"));
        } else {
            header("Location: edit.php?id=$id&tip=$tip&error=" . urlencode("Došlo je do greške pri ažuriranju!"));
        }
        exit();
    }
}
